...
#include <Arduino.h>
#define DO_INIT() pinMode(MISO, INPUT)
#define DO digitalRead(MISO)
#define DI_INIT() pinMode(MOSI, OUTPUT)
#define DI_H() digitalWrite(MOSI, HIGH)
#define DI_L() digitalWrite(MOSI, LOW)
#define CK_INIT() pinMode(SCK, OUTPUT)
#define CK_H() digitalWrite(SCK, HIGH)
#define CK_L() digitalWrite(SCK, LOW)
#define CS_INIT() pinMode(3, OUTPUT)
#define CS_H() digitalWrite(3, HIGH)
#define CS_L() digitalWrite(3, LOW)
...
)
{
	...
	do {
		d = *buff++;	
		if (d & 0x80) DI_H(); else DI_L();	
		CK_H(); CK_L();
		if (d & 0x40) DI_H(); else DI_L();	
		CK_H(); CK_L();
		if (d & 0x20) DI_H(); else DI_L();	
		CK_H(); CK_L();
		if (d & 0x10) DI_H(); else DI_L();	
		CK_H(); CK_L();
		if (d & 0x08) DI_H(); else DI_L();	
		CK_H(); CK_L();
		if (d & 0x04) DI_H(); else DI_L();	
		CK_H(); CK_L();
		if (d & 0x02) DI_H(); else DI_L();	
		CK_H(); CK_L();
		if (d & 0x01) DI_H(); else DI_L();	
		CK_H(); CK_L();
	} while (--bc);
}
